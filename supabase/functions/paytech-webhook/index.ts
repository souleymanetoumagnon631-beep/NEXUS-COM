...
function corsHeaders(allowedOrigin = "*") {
  // En production, restreindre à l'origine du site NEXUS
  const origin = allowedOrigin !== "*" ? allowedOrigin : Deno.env.get("NEXUS_ORIGIN") || "*";
  return {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // NOTE : PAYTECH_WEBHOOK_SECRET est OBLIGATOIRE en production.
    // Sans lui, le webhook rejette toutes les requêtes.
    const WEBHOOK_SECRET = Deno.env.get("PAYTECH_WEBHOOK_SECRET");
    const signatureHeader = req.headers.get("X-PayTech-Signature");

    if (!WEBHOOK_SECRET) {
      console.error("[PayTech Webhook] PAYTECH_WEBHOOK_SECRET non configuré. Rejet de toutes les requêtes.");
